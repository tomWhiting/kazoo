//! IPC integration for kazoo-mini.
//!
...
impl HubLink {
    ...
    const NAME: &'static str = "kazoo-mini";
    ...
    pub fn new(channel_count: u8, sample_rate: u32, buffer_size: u32) -> Self {
        ...
        // Run the blocking connection handshake on a dedicated thread
        // with a hard timeout so the UI thread is never stalled.
        let (tx, rx) = std::sync::mpsc::channel();
        let _handle = std::thread::Builder::new()
            .name("kazoo-mini-ipc-connect".into())
            .spawn(move || {
                let result =
        ...
        // Wait up to CONNECT_TIMEOUT for the handshake to complete.
        let client = rx
