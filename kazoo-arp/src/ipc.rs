...
#![allow(dead_code)] // Hub infrastructure — used when standalone connects to hub.
...
impl HubLink {
    ...
    const NAME: &'static str = "kazoo-arp";
    ...
    pub fn new(channel_count: u8, sample_rate: u32, buffer_size: u32) -> Self {
        ...
        let _handle = std::thread::Builder::new()
            .name("kazoo-arp-ipc-connect".into())
            .spawn(move || {
                let result =
    ...
    /// Clone of the connection state flag for external monitoring (e.g. UI thread).
    #[must_use]
